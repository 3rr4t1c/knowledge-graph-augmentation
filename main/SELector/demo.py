...
from selector_extended import SELector
import pandas as pd
import json
...
lector_TT = 'data/lector_example_train/text_triples_preprocessed.tsv'
...
fewrel_GT = 'data/fewrel_test/fewrel_translated_gt_text_preprocessed.tsv'

# Link prediction model path
lp_model_file = 'path/to/link/prediction/model.pt' # Non implementato
...
answ = input(f'* Vuoi impostare i parametri manualmente? y / n (usa default) > ')
if answ[0].lower() == 'y':
    ...
    type_remapping_opt = input('Riassegnare i tipi a grana larga? (verrano usati comunque i tipi) y / n (default = No) > ')
    ...
    enable_LP_opt = input('Abilitare link prediction? y / n (default = No) > ')
    if enable_LP_opt is not '' and enable_LP_opt[0].lower() == 'y':
        enable_LP = lp_model_file
        print('non implementato')
        exit()


# Mostra informazioni
print('\n* Avvio demo con parametri:')
...
print(f'Ri-assegnazione tipi: {"Abilitata" if type_remapping else "Disabilitata"}')
print(f'Link Prection: {"Abilitata" if enable_LP else "Disabilitata"}')
